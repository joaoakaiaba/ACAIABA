  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-gray-100 pb-5">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight uppercase">Registro de Auditoria Administrativa</h1>
          <p className="text-sm text-gray-500 mt-1">Monitore alterações de preços, movimentações de estoque, logins e ações administrativas.</p>
        </div>
      </div>
      ...
      {audits.length > 0 ? (
                ...
                {audits.map((audit) => (
                  <tr key={audit.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        <span className="text-xs font-black text-slate-950 uppercase tracking-tight">{audit.action}</span>
                        <p className="text-[10px] text-amber-600 font-extrabold uppercase mt-0.5">{audit.entity}</p>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-xs font-mono text-gray-600 max-w-md truncate">
                      {audit.details}
                    </td>
                  </tr>
                ))}
        ...
        <div className="rounded-2xl border border-dashed border-gray-200 py-24 text-center px-4 max-w-2xl mx-auto">
          ...
          <h3 className="mt-4 text-lg font-bold text-gray-800 uppercase tracking-tight">Sem atividades auditadas</h3>
