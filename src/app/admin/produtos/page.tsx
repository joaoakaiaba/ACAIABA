  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-gray-100 dark:border-slate-800 pb-5">
        <div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight uppercase">Gerenciamento de Produtos</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Crie, edite, duplique ou altere o status de produtos do catálogo.</p>
        </div>
      ...
      {products.length > 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-100 text-left text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {products.map((p) => (
                  <tr key={p.id} className="hover:bg-slate-50 dark:bg-slate-900 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-600 dark:text-gray-300">
                      {p.categoryName}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        <p className="font-extrabold text-slate-950 dark:text-white font-mono">R$ {p.price.toFixed(2)}</p>
                        {p.promotionalPrice && (
                          <span className="text-[10px] text-emerald-600 font-bold font-mono">Promo: R$ {p.promotionalPrice.toFixed(2)}</span>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-center font-bold font-mono text-slate-800 dark:text-gray-100">
                      {p.stock}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-xs font-semibold">
                      <div className="flex justify-end gap-2">
                        <Link
                          ...
                          className="h-8 w-8 flex items-center justify-center rounded-lg border border-gray-200 dark:border-slate-700 bg-white hover:bg-gray-50 dark:bg-slate-900 dark:hover:bg-slate-800 text-gray-700 dark:text-gray-300 hover:text-amber-600 shadow-sm transition-all"
                          title="Editar"
                        >
                          <Edit className="h-4 w-4" />
                        </Link>
                        <ProductRowActions productId={p.id} active={p.isActive} />
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div className="rounded-2xl border border-dashed border-gray-200 dark:border-slate-700 py-24 text-center px-4 max-w-2xl mx-auto">
          ...
          <h3 className="mt-4 text-lg font-bold text-gray-800 dark:text-gray-100 uppercase tracking-tight">Sem produtos cadastrados</h3>
