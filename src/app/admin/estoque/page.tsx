  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-gray-100 dark:border-slate-800 pb-5">
        <div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight uppercase">Controle de Estoque Transacional</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Monitore níveis de estoque, gerencie quantidades mínimas e execute movimentações.</p>
        </div>
      </div>
      ...
      {stock.length > 0 ? (
            ...
            <table className="min-w-full divide-y divide-gray-100 text-left text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {stock.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-slate-50 dark:bg-slate-900 dark:hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap font-semibold">
                        <div>
                          <p className="font-mono text-xs uppercase text-slate-900 dark:text-white">SKU: {item.sku}</p>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-center font-bold font-mono text-gray-500 dark:text-gray-400">
                        {item.minStock}
                      </td>
        ...
        <div className="rounded-2xl border border-dashed border-gray-200 dark:border-slate-700 py-24 text-center px-4 max-w-2xl mx-auto">
          ...
          <h3 className="mt-4 text-lg font-bold text-gray-800 dark:text-gray-100 uppercase tracking-tight">Sem estoque registrado</h3>
